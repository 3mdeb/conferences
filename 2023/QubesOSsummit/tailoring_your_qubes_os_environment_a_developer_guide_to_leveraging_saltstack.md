...
_I am not a SaltStack sage or a Qubes OS guru. The contents of this presentation
are more of a curious exploration than an expert guide. If you decide to use any
materials, knowledge, or scripts shared today and somehow trigger a digital
Armageddon, start a nuclear showdown, or just plain old crash your system,
remember, it's not on me!_
...
.center[.image-10[![](/img/qubesos_logo.png)]
.image-50[![](/img/saltstack.png)]]

- **SaltStack** is Apache 2.0 licensed, written in Python configuration
...
- Utilizes a central management server (Salt Master) to communicate with and
  ...
  operation.
  + Masterless configuration is used by Qubes OS
- Employs a declarative language in YAML to define desired system
  configurations, ensuring consistency and compliance across the infrastructure.
- **SaltStack** was acquired by VMware in 2020
- **SaltStack** integration was introduced to Qubes R3.1 (Q1'2016)
...
- There is no officially supported way to rollback whole dom0, just home
  ...
  + there are many threads and complex discussion aroudnd dom0 lvm and
    snapshotting
  + `tasket` and `tlaurion` have some success, which leverage `wyng-backup`, but
    whole solution is not described well enough, so casual user like me could
    leverage that
...
.center.image-10[![](/img/appvm-black.svg) ![](/img/templatevm-black.svg)
![](/img/adminvm-black.svg)]

- In case of AppVMs, TemplateVMs and dom0 home directory:
  + Recovery from backup for VMs requiring complex manual provisioning, which is
...
- Pillars, formulas and grains and other Salt terminology unnecessarily
...
- `qubesctl` is a tool used to manage and configure Qubes OS using SaltStack. In
  essence it is inter-changeable and an alias for `salt-call --local`.
...
- **Desired outcome** - You specify what you want the final configuration to
  look like, not how to get there. Framework, in this case SaltStack, takes care
  of everything else.
...
- Qubes organization (reading
  [How to organize your qubes](https://www.qubes-os.org/doc/how-to-organize-your-qubes/)
  highly recommended)
...
- IMHO building embedded and firmware projects on computer you use for work does
  not make sense. I argue for external build system in intranet.
...
- Download, install and configure VMs using pillar data to define default names
...
- Best what I was able to find is `xfconf-query` a tool created for querying and
  setting Xfce configuration database.
...
- Clean Qubes OS installation has some VMs created (personal, work, vault,
  untrusted).
...
- Apparently 32G translate to `32*10^9`, but `qvm-volume` set power of two, so
  calling this second time return error: `32*10^9 < 32002539520`
...
- `qvm.service` is used in similar way as `qvm-service` to manage Qubes OS
  specific services started in VMs
...
- Easiest way seem to be to deliver on USB/storage and then copy content to
  dom0.
...
- Before kick any Salt script let's make sure that dom0's and AppVM templates
  are up to date:
...
- Logs from template and VM configuration can be found in
  `/var/log/qubes/mgmt-VMNAME.log`

---

# SOP: Structure of script package

- `*.sls` and `*.top` are named after the target names: dom0, debian-12-minimal,
  dev, etc.
...
.center[_No Top file or master_tops data matches found. Please see master log
for details._]

- There is nothing in `/var/log/salt/master`
- It seems that new Qubes OS consume top files differently because there is no
  `/srv/salt/_tops/base/topd.top` symlink to `/srv/salt/topd/init.top`
...
- Other issues:
  + Please note `init.sh` rely on symlinks, if `files` or other of your
    directory already contain broken symlinks it can lead to very weird and hard
    to track errors.
...
- To really minimize storage footprint, configuration time and bandwidth
  required for package installation we should first modify `debian-12-minimal`
  with all necessary common packages between `comm` and `dev`. Then clone it to
  create `debian-12-{comm,dev}`
...
- Scripts are typically modified in dom0
  + whenever you need to improve TemplateVM you apply change and test it, if it
...
- Avoid laziness, which lead to manual modification and not recording steps that
  lead to current state of AppVM
- For changes which are not persistence (require template modification) this is
...
- Organization of `*.sls` and `*.top` files could be way better, especially
  considering fact that top file can reference multiple individual states from
  multiple sls files
...
- Why we should base on debian-12-xfce?
  + isn't it bloated by default?
- Dealing with Qubes OS UI without mouse:
  [Xfce Cheat Sheet](https://defkey.com/xfce-shortcuts?orientation=landscape&cellAlternateColor=%23d6ffef&showPageNumber=true&pdf=True&showPageNumber=false)
