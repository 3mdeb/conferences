...
* **S-RTM** - _Static Root of Trust for Measurement_
    - _Root of Trust_ because it is unconditionally trusted component, for
      which misbehavior cannot be detected
    - _RoT for Measurement_ because it is responsible for initial integrity
      measurement
    - _Static_ because shielded location (PCR) initialization and initial integrity
      measurement occurs only on platform reset
...
* What options of enabling TPM in QEMU we have?
    ...
    - Qubes OS R4.0 use QEMU 3.0.0 which may limit our options for TPM, but
      R4.1 will use QEMU 4.2.0, which should cover most the bases
* Boot firmware:
...
* For x86 it can be:
    ...
    - fw_cfg interface - only for Physical Presence Interface (PPI) memory, TPM version, PPI version
...
* Interesting point is that change to image/boot firmware can be introduced
  only in dom0, what implies that dom0 is in TCB and should be measured ergo
  should be stateless
